...
Run with:
    uv run python tests/guidance/hparam_search_smiley_optuna_600steps.py
"""
...
STUDY_NAME = "smiley_600steps"
STUDY_PATH = f"sqlite:///{OUT_DIR}/hparam_search_smiley_optuna_600steps.db"
CSV_PATH = f"{OUT_DIR}/hparam_search_smiley_optuna_600steps_results.csv"
...
# with margin for the search to move beyond what's already been tried. lr's
# lower bound is dropped a further ~2x from the 200-step version, since lr is
# expected to scale roughly inversely with EULER_STEPS (~0.0037 predicted at
# 600 steps if the 200-step optimum's ~0.011 scales that way) -- see module
# docstring.
BOUNDS = {
    "gamma_value": (0.3, 6.0),
    "gamma_hi": (0.8, 6.0),
...
def main() -> None:
    ...
    if len(study.trials) == 0:
        ...
        # Not a warm-start import (see module docstring) -- just two informed
        # first guesses derived from the euler_steps=200 search's best trial
        # (gamma_hi=2.03, gamma_lo_slope=0.341, gamma_threshold=0.203,
        # w_terminal=13.2, w_control=3.4e-05), with lr scaled down by the
        # euler_steps ratio (200->600 = 3x) per the inverse-scaling comment.
        # These still get *evaluated* at 600 steps, not assumed -- they just
        # give TPE a plausible place to start instead of pure random init.
        study.enqueue_trial(
            {
                "gamma_mode": "schedule",
        ...
        study.enqueue_trial(
            {
                ...
                "gamma_lo_slope": 0.34118882821876156,
                "gamma_threshold": 0.20308202926093588,
                "lr": 0.011186441171591767,  # unscaled, in case the 1/steps scaling doesn't hold exactly
                "w_terminal": 13.249253833502488,
                "w_vf": 0.0,
                "w_control": 3.390747222335388e-05,
            }
        )
        print(f"Cold-started study with {len(study.trials)} trials queued (0 warm-started + 2 scaled seed guesses).")
    else:
