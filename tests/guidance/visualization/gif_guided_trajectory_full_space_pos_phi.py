...
phi/psi Ramachandran space under the "pos_phi" objective (one-sided penalty,
zero cost once phi > 0), using the same guidance hyperparameters currently set
in ``plot_guided_euler_ramachandran.py`` (EULER_STEPS, GUIDANCE_GAMMA,
GUIDANCE_LR, GUIDANCE_W_TERMINAL, etc.) -- only the cost function differs from
that file's current OBJECTIVE="smiley" setting, since pos_phi needs its own
cost (one_sided_quadratic_penalty on phi, no psi/eye/mouth geometry at all).

Same visualization style as ``gif_guided_trajectory_full_space.py``: the whole
[-pi, pi] x [-pi, pi] torus, red scatter -- with enough samples the phi>0
half-plane should visibly empty out into the other half as the samples
converge, relative to the thin dashed black line marking the phi=0
threshold. The background is a density plot of the *unguided* equilibrium
distribution (a fresh, quick unguided Euler pass with the same model, same
step count as ``plot_unguided_euler_ramachandran.py``), drawn with the same
style as ``plot_ramachandran.py``'s own density plots (hist2d, LogNorm,
default viridis colormap, at 40% alpha).
...
Run with:
    uv run python tests/guidance/visualization/gif_guided_trajectory_full_space_pos_phi.py
"""
...
from transferable_samplers.guidance.costs import one_sided_quadratic_penalty
...
GIF_PATH = f"{OUT_DIR}/guided_trajectory_pos_phi_full_space.gif"
...
def main() -> None:
    ...
    print("=== Guidance hyperparameters (from plot_guided_euler_ramachandran.py, pos_phi cost) ===")
    ...
    model.guidance_cost_fn = pos_phi_cost_fn
    ...
    # equilibrium distribution, plus a thin dashed line marking the phi=0
    # threshold -- the phi>0 half-plane should visibly empty out.
    ...
    ax.axvline(0.0, color="black", linewidth=0.8, linestyle="--", zorder=4)
